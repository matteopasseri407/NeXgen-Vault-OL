...
| `render.py` | executor | generates MCP configs from the manifest, preserves unknown entries by default, removes only explicit `retired_servers`, and computes drift; `--revert` restores a config from its most recent backup; `--adopt` drafts manifest entries for out-of-manifest live servers |
