...
- **Wrapper `firecrawl-local`** (only for L0/deterministic scripts, no MCP): calls `/v2/scrape` and `/v2/search` defaulting to the tunnel URL. Source: `03-INFRA/scripts/firecrawl-local.sh` on Linux/macOS and `03-INFRA/scripts/firecrawl-local.ps1` on Windows.
