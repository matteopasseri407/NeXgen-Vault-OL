...
5. **Keep coherence.** Update `99-INDEX/note-index.md`, repair `[[links]]` you touched, and record the tranche in `99-INDEX/vault-cleanup-backlog.md` (what you decided and why — the backlog is the memory that stops the next run redoing this). This is your own qualitative decision entry; it's separate from the structured line the wrapper adds afterward (step 6).

6. **Commit.** Atomic commits per tranche (clear message), then push (unless `GROOM_NOPUSH=1`). Do not also append a one-line summary yourself — the wrapper (`vault-groom.sh`/`.ps1`, via `vault_groom_audit.py`) appends a structured record to the backlog automatically once your run returns, built from the real git history it produced (commits, files touched), not from a self-report. That record is also saved in full as JSON under the state dir (default `~/.local/state/vault-groom/`), alongside the exact tranche text the user approved before you ever started writing.
...
- **Guarded by default, not a two-command dance.** Running `vault-groom` with no argument does the whole guarded flow in one invocation: propose a tranche (read-only), print it in full, and stop for a typed `yes` — anything else aborts with zero changes. Only after that confirmation does it re-invoke the runner in write mode, and it hands that write pass the EXACT approved text (with its sha256) instead of letting it re-derive its own tranche. `vault-groom preview` is the read-only-only variant: shows a proposal and stops, no confirmation prompt, nothing ever executes from it.
